...
function get_default_images(need_change=true) {
    $.ajax({
        ...
        data: {
            'categories': JSON.stringify($('#select-tools').val())
        },
        dataType: 'json',
        success: function(data) {
            for (let j = 1; j < 4; j ++) {
                if (data['category_' + j]['name'] !== '') {
                    $('.column_' + j).show();
                    ...
                    for (let i = 0; i < 3; i++) {
                        $selector.append('<div en_name='+ data['category_' + j]['name']
                            ...
                            data['category_' + j]['list'][i] + '"></div>');
                    }
                    $selector.find('.rec_im:first').addClass('selected_img');
                } else {
                    $('.column_' + j).hide();
                }
            }
...
/**
 * Смена аватара на событии
 *
 */
function change_event_avatar() {
    let json = {};
    $('.selected_img', '.recommend_image_block').each(function(index) {
        json[$(this).attr('en_name')] = $(this).find('img').attr('src');
    });
